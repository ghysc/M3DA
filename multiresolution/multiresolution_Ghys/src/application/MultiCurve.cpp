...
void MultiCurve::analysisStep() {
   ...
   */
  _detail[level] = coarse;
  for(int i=0; i<coarse.size(); i++) {
      Vector3 meanA = (-_currentCurve[(2*i)%n] + 3*_currentCurve[(2*i+1)%n] + 3*_currentCurve[(2*i+2)%n] -_currentCurve[(2*i+3)%n]) / 4;
      coarse[i] = meanA;
      Vector3 meanB = ( _currentCurve[(2*i)%n] - 3*_currentCurve[(2*i+1)%n] + 3*_currentCurve[(2*i+2)%n] -_currentCurve[(2*i+3)%n]) / 4;
      _detail[level][i] = meanB;
  }
  /* end TODO
   */
