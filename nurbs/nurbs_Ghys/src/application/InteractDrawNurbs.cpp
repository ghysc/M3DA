...
bool InteractDrawNurbs::computeCurve() {
   ...
   */
  double step = (double) (_nurbs->endInterval(D_U)-_nurbs->startInterval(D_U))/(_drawNbPts-1);
  if(step < 0) return false;
  double u = _nurbs->startInterval(D_U);
  Vector3 v;
  for(int i=0; i<_drawNbPts; i++, u+=step) {
      v = _nurbs->pointCurve(u);
      _draw[i] = v;
  }
...
vector<Vector2> InteractDrawNurbs::basisRepresentation(int k,int p,const std::vector<double> &knot) {
   ...
   */
  double step = (double) 1/(nbPoint-p+1);
  double u, nkp;

  for(int i=0; i<nbPoint; i++) {
      u = step*i;
      nkp = _nurbs->evalNkp(k, p, u, knot);
      if(u >= knot[k] && u <= knot[k+p+1]) {
